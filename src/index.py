...
def fig_time_evolution(x, psi_t_list, t_list, frame_duration_ms=90, title="Wave-Packet Evolution"):
    ...
    fig.update_layout(**merge_plot_layout(
        title=dict(text=f"{title} (Press Play)", font=dict(family="Space Mono", size=13, color="#00d4ff")),
        xaxis_title="x (a₀)",
        yaxis_title="|ψ(x,t)|²",
        height=420,
        updatemenus=[dict(
...
# Tabs
(tab_eigen, tab_packet, tab_super, tab_analysis) = st.tabs(
    ["  Eigenstates  ", "  Wave Packet  ", "  Superposition  ", "  Analysis  "]
)
